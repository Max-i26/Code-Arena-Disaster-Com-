    ...
    <div className="space-y-6">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-5 flex flex-wrap items-center justify-between gap-4 shadow-xl">
        <div className="flex items-center space-x-3.5">
          <div className="p-3 rounded-2xl bg-blue-600/20 text-blue-400 border border-blue-500/30 shadow">
            <Radio className="w-6 h-6 animate-pulse text-blue-400" />
          </div>
          <div>
            <h2 className="text-lg font-extrabold text-white tracking-wide">Citizen Self-Care & Emergency Channel</h2>
            <p className="text-sm text-slate-300 mt-0.5 font-medium">Report flash floods, blocked roads, or request emergency evacuation.</p>
          </div>
        </div>

        {/* Tab Switcher */}
        <div className="flex items-center space-x-1.5 bg-slate-950 p-1.5 rounded-2xl border border-slate-800 shadow-inner">
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all ${
              activeTab === 'REPORT' ? 'bg-blue-600 text-white shadow-lg' : 'text-slate-300 hover:text-white'
            }`}
          >
            Report Hazard
          </button>
          <button
            ...
            className={`px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all ${
              activeTab === 'RESCUE' ? 'bg-rose-600 text-white shadow-lg' : 'text-slate-300 hover:text-white'
            }`}
          >
          ...
          <button
            ...
            className={`relative px-4 py-2.5 rounded-xl text-xs sm:text-sm font-bold transition-all ${
              activeTab === 'COMMUNITY_VERIFY' ? 'bg-amber-600 text-white shadow-lg' : 'text-slate-300 hover:text-white'
            }`}
          >
      ...
      {activeTab === 'REPORT' && (
        <div className="space-y-6">
          ...
          {/* Submission Form */}
          <div className="lg:col-span-7 bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl space-y-5">
            ...
            <form onSubmit={handleSubmitReport} className="space-y-4 text-sm">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-slate-300 block mb-1.5 font-semibold">Your Full Name</label>
                  <input
                    type="text"
                    required
                    value={userName}
                    onChange={(e) => setUserName(e.target.value)}
                    className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-white font-medium focus:outline-none focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400"
                  />
                </div>
                <div>
                  <label className="text-slate-300 block mb-1.5 font-semibold">Contact Phone Number</label>
                  <input
                    type="text"
                    required
                    value={contactPhone}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="text-slate-300 block mb-1.5 font-semibold">Hazard Category</label>
                  <select
                    value={hazardType}
                    onChange={(e) => setHazardType(e.target.value as HazardType)}
                    className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-white font-medium focus:outline-none focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400"
                  >
                    <option value="FLOOD" className="bg-slate-900">🌊 Urban Flooding / Waterlogging</option>
                    <option value="FALLEN_TREE" className="bg-slate-900">🌳 Fallen Tree Obstruction</option>
                    <option value="LANDSLIDE" className="bg-slate-900">⛰️ Landslide / Mudflow</option>
                    <option value="BLOCKED_DRAIN" className="bg-slate-900">🚧 Blocked Storm Drain / Culvert</option>
                    <option value="DOWNED_POWERLINE" className="bg-slate-900">⚡ Downed Electric Powerline</option>
                  </select>
                </div>
                <div>
                  <div className="flex items-center justify-between mb-1.5">
                    <label className="text-slate-300 font-semibold">City Ward / Location</label>
                    <button
                      type="button"
                  ...
                  <select
                    ...
                    className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-white font-medium focus:outline-none focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400"
                  >
                    <option value="AUTO_GPS" className="bg-slate-900">📍 Auto-Detect Current GPS Location</option>
                    {state.wards.map(w => (
                      <option key={w.id} value={w.id} className="bg-slate-900">{w.name}</option>
                    ))}
                  </select>
                  {detectedGpsCoords && (
                    <div className="mt-1.5 text-xs text-emerald-400 font-mono font-bold flex items-center space-x-1">
              ...
              <div>
                <label className="text-slate-300 block mb-1.5 font-semibold">Street / Road Name</label>
                <input
                  type="text"
                  required
                  value={roadName}
              ...
              {/* Photo Selector */}
              <div>
                <label className="text-slate-300 block mb-1.5 font-semibold">Attach Photo Proof (Camera / Device File or Preset)</label>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2.5">
                  {samplePhotos.map((p, idx) => (
                    <button
                      ...
                      className={`text-left p-2.5 rounded-xl border text-xs transition flex items-center space-x-2.5 font-medium ${
                        imageUrl === p.url
                          ? 'bg-cyan-950/80 border-cyan-400 text-cyan-200'
                          : 'bg-slate-950 border-slate-800 text-slate-300 hover:border-slate-700'
                      }`}
                    >
                      <img src={p.url} alt="" className="w-10 h-10 rounded-lg object-cover flex-shrink-0" />
              ...
              <div>
                <label className="text-slate-300 block mb-1.5 font-semibold">Detailed Situation Description</label>
                <textarea
                  rows={2}
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  placeholder="Describe flood depth, blocked lanes, stalled vehicles, rising water..."
                  className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-white font-medium focus:outline-none focus:border-cyan-400 focus:ring-1 focus:ring-cyan-400 resize-none"
                />
              </div>

              {/* Rescue Checkbox */}
              <div className="bg-rose-950/40 border border-rose-800/60 p-4 rounded-xl flex items-center justify-between">
                <div className="flex items-center space-x-3">
                  <LifeBuoy className="w-6 h-6 text-rose-400 flex-shrink-0" />
                  <div>
                    <span className="font-bold text-white text-sm block">Are you trapped or in need of immediate evacuation?</span>
                    <span className="text-xs text-slate-300">Routes rescue boat/pump squad and matches to nearest shelter with beds.</span>
                  </div>
                </div>
                <input
                  type="checkbox"
                  ...
                  className="w-5 h-5 rounded border-rose-800 text-rose-600 focus:ring-0 cursor-pointer"
                />
              </div>

              {needsRescue && (
                <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 flex items-center justify-between">
                  <span className="text-slate-300 font-semibold text-sm">Household members stranded:</span>
                  <input
                    type="number"
                    min={1}
                    max={20}
                    ...
                    className="w-24 bg-slate-900 border border-slate-700 rounded-xl px-3 py-1.5 text-center text-white font-bold text-base"
                  />
                </div>
              )}
              ...
              <button
                ...
                className="w-full bg-gradient-to-r from-cyan-600 to-blue-600 hover:from-cyan-500 hover:to-blue-500 text-white font-extrabold py-3.5 px-6 rounded-xl shadow-xl shadow-blue-500/20 flex items-center justify-center space-x-2.5 transition text-sm disabled:opacity-50"
              >
          ...
          <div className="lg:col-span-5 space-y-4">
            {submittedResult ? (
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4 animate-in fade-in">
                <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                  <div className="flex items-center space-x-2">
                    ...
                    <h4 className="font-extrabold text-white text-sm uppercase">Instant Diagnostic</h4>
                  </div>
                ...
                {submittedResult.verdict?.reasoningChain && (
                  <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2">
                    <span className="text-xs font-bold text-cyan-400 uppercase tracking-wider block">Reasoning Chain:</span>
                    <ul className="text-xs text-slate-200 list-disc list-inside space-y-1 leading-relaxed">
                ...
                {(() => {
                  ...
                  return (
                    <div className="bg-purple-950/60 border border-purple-800/80 p-4 rounded-xl space-y-2.5">
                      <div className="flex items-center justify-between text-xs font-bold text-purple-300">
                        <span>🏨 Nearest Shelter Accommodating {reqCount} Citizen(s):</span>
                        <span className="bg-emerald-950 text-emerald-300 border border-emerald-800 px-2.5 py-0.5 rounded text-xs font-mono font-bold">
                          {nearestShelter.freeBeds} BEDS FREE
                        </span>
                      </div>
                      <div className="space-y-1.5 text-xs">
                        <div className="font-bold text-white text-base flex justify-between items-center">
                          <span>{nearestShelter.name}</span>
                          <span className="text-cyan-400 font-mono text-xs font-bold">{nearestShelter.distanceKm.toFixed(1)} km away</span>
                        </div>
                        <div className="text-slate-300 flex justify-between font-mono text-xs">
                          <span>Shelter Hotline:</span>
                          <span className="text-cyan-300 font-bold">{nearestShelter.contactPhone}</span>
                        </div>
                        <div className="text-xs text-emerald-300 font-mono pt-1 border-t border-purple-900/60 flex items-center space-x-1 font-semibold">
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl text-center space-y-3">
                <div className="w-14 h-14 rounded-2xl bg-cyan-950/80 border border-cyan-800/60 text-cyan-400 flex items-center justify-center mx-auto shadow">
                  <Shield className="w-7 h-7" />
                </div>
                <h4 className="font-bold text-white text-base">Real-time Automated Pipeline</h4>
        ...
        <div className="space-y-6">
            ...
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <div>
                <h3 className="text-base font-extrabold text-white uppercase tracking-wider flex items-center space-x-2.5">
                  ...
                  <span>Crowdsourced Verification Requests ({pendingVerificationCases.length})</span>
                </h3>
              ...
              <span className="text-xs text-slate-400 font-mono">Verification Loop</span>
            </div>
            ...
            {pendingVerificationCases.length === 0 ? (
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {pendingVerificationCases.map((c) => {
                  ...
                  return (
                    <div key={c.id} className="bg-slate-950 border border-slate-800 p-5 rounded-2xl space-y-3 shadow">
                      ...
                      <p className="text-sm text-slate-200 leading-relaxed">"{c.description || 'Reported incident'}"</p>
                      ...
                      <div className="flex items-center space-x-2.5 pt-2">
                        <button
                          ...
                          className="flex-1 bg-emerald-600 hover:bg-emerald-500 text-white py-2.5 rounded-xl text-xs font-bold transition shadow"
                        >
                          ✓ Confirm Hazard
                        </button>
                        <button
                          onClick={() => handleNearbyVerify(c.id, false)}
                          className="flex-1 bg-rose-600 hover:bg-rose-500 text-white py-2.5 rounded-xl text-xs font-bold transition shadow"
                        >
                          ✕ False Alarm
                        </button>
                      </div>
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            ...
            {state.cases.length === 0 ? (
              <div className="text-center py-8 text-slate-400 text-sm">No active reports recorded yet. Be the first to submit a report on the Report Hazard tab!</div>
            ) : (
                ...
                {state.cases.slice(0, 9).map((c) => {
                  ...
                  return (
                    <div key={c.id} className="bg-slate-950 border border-slate-800 p-5 rounded-2xl space-y-3 shadow">
                      ...
                      <p className="text-sm text-slate-200 line-clamp-2 leading-relaxed">"{c.description || 'Reported hazard incident'}"</p>
                      <div className="flex items-center justify-between text-xs text-slate-300 font-mono">
                        <span>{c.location?.roadName || 'City Corridor'}</span>
                        <span className="text-cyan-400 font-bold">{distKm} km away</span>
                      </div>
                      <div className="flex items-center justify-between pt-2 border-t border-slate-800 text-xs">
                        <span className={c.roadClosed ? 'text-rose-400 font-bold' : 'text-emerald-400 font-bold'}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
            ...
            <div className="lg:col-span-7 bg-slate-900 border border-slate-800 rounded-2xl p-6 sm:p-8 shadow-xl space-y-5">
              <div className="flex items-center space-x-3.5 border-b border-slate-800 pb-4">
                <div className="p-3 rounded-2xl bg-rose-950 border border-rose-800 text-rose-400 shadow">
                  <LifeBuoy className="w-6 h-6 animate-pulse" />
                </div>
                <div>
                  <h3 className="text-base font-extrabold text-white uppercase tracking-wider">Request Emergency Evacuation & Relief</h3>
                  <p className="text-xs sm:text-sm text-slate-300 mt-0.5">Submit stranded family details to auto-reserve beds at nearest shelter and route rescue boats.</p>
                </div>
              </div>

              <form onSubmit={handleSubmittingRescue} className="space-y-4 text-sm">
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="text-slate-300 block mb-1.5 font-semibold">Citizen Full Name</label>
                    <input
                      type="text"
                      required
                      value={rescueName}
                      onChange={(e) => setRescueName(e.target.value)}
                      className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-white font-medium focus:outline-none focus:border-rose-400 focus:ring-1 focus:ring-rose-400"
                    />
                  </div>
                  <div>
                    <label className="text-slate-300 block mb-1.5 font-semibold">Emergency Phone Hotline</label>
                    <input
                      type="text"
                      required
                      value={rescuePhone}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <button
                        ...
                        onClick={handleDetectGpsLocation}
                        disabled={isDetectingGps}
                        className="text-xs text-rose-400 hover:text-rose-300 font-bold flex items-center space-x-1 transition"
                      >
                  ...
                  <div>
                    <label className="text-slate-300 block mb-1.5 font-semibold">Stranded Family Members</label>
                    <input
                      type="number"
                      min={1}
                      max={20}
                      ...
                      className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-white font-bold text-center focus:outline-none focus:border-rose-400 focus:ring-1 focus:ring-rose-400"
                    />
                  </div>
                </div>

                <div>
                  <label className="text-slate-300 block mb-1.5 font-semibold">Street Address / Road Name</label>
                  <input
                    type="text"
                    required
                    value={rescueRoad}
                ...
                <div>
                  <label className="text-slate-300 block mb-1.5 font-semibold">Special Assistance / Medical Needs</label>
                ...
                <div>
                  <label className="text-slate-300 block mb-1.5 font-semibold">Urgency Notes / Physical Obstruction</label>
                  <textarea
                    rows={2}
                    value={rescueNotes}
                    onChange={(e) => setRescueNotes(e.target.value)}
                    placeholder="Describe flood depth, roof level, medical emergencies..."
                    className="w-full bg-slate-950 border border-slate-700 rounded-xl px-4 py-2.5 text-white font-medium focus:outline-none focus:border-rose-400 focus:ring-1 focus:ring-rose-400 resize-none"
                  />
                </div>
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-rose-600 to-red-600 hover:from-rose-500 hover:to-red-500 text-white font-extrabold py-3.5 px-6 rounded-xl shadow-xl shadow-rose-500/20 flex items-center justify-center space-x-2.5 transition text-sm disabled:opacity-50"
                >
            ...
            <div className="lg:col-span-5 space-y-4">
              {rescueResult ? (
                <div className="bg-slate-900 border border-rose-700/80 rounded-2xl p-6 shadow-xl space-y-4 animate-in fade-in">
                  <div className="flex items-center justify-between border-b border-slate-800 pb-3">
                    <div className="flex items-center space-x-2">
                      <CheckCircle2 className="w-5 h-5 text-emerald-400" />
                      <h4 className="font-extrabold text-white text-sm uppercase">Shelter Allocation Confirmed</h4>
                    </div>
                  ...
                  <div className="space-y-3 text-sm">
                    {rescueResult?.match?.shelter ? (
                      <div className="bg-slate-950 p-4 rounded-xl border border-slate-800 space-y-2.5">
                        ...
                        <div className="text-slate-300">Ward: {rescueResult.match.shelter.wardId}</div>
                        <div className="flex items-center justify-between text-slate-200 font-mono">
                          <span>Allocated Beds:</span>
                          <span className="font-bold text-emerald-400 text-base">{rescueResult.request?.householdCount || rescueCount} Beds Reserved</span>
                        </div>
                        <div className="flex items-center justify-between text-slate-200 font-mono">
                          <span>Shelter Hotline:</span>
                          <span className="font-bold text-cyan-400 text-base">{rescueResult.match.shelter.contactPhone || '+94 11 291 3344'}</span>
                        </div>
                        <div className="pt-2 border-t border-slate-800 text-xs text-emerald-300 flex items-center space-x-2 font-semibold">
                          <CheckCircle2 className="w-4 h-4 text-emerald-400 flex-shrink-0" />
                          <span>Evacuation boat unit dispatched to {rescueResult.request?.location?.roadName || rescueRoad}</span>
                        </div>
                      </div>
                    ) : (
                      <div className="bg-amber-950/60 p-4 rounded-xl border border-amber-800 text-amber-200 text-xs font-bold">
                ...
                <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl text-center space-y-3">
                  <div className="w-14 h-14 rounded-2xl bg-rose-950/80 border border-rose-800/60 text-rose-400 flex items-center justify-center mx-auto shadow">
                    <LifeBuoy className="w-7 h-7" />
                  </div>
                  <h4 className="font-bold text-white text-base">Automated Evacuation & Capacity Matching</h4>
                  <p className="text-sm text-slate-300 leading-relaxed font-normal">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 shadow-xl space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 pt-1">
              {state.shelters.map((sh) => (
                <div key={sh.id} className="bg-slate-950 border border-slate-800 p-5 rounded-2xl space-y-3 shadow">
                  <div className="flex items-center justify-between">
                  ...
                  <div className="text-xs text-slate-300">Ward: {sh.wardId}</div>
                  ...
                  <div className="text-xs text-slate-300 font-mono pt-1">
                    Hotline: <span className="text-cyan-300 font-bold">{sh.contactPhone}</span>
                  </div>
                </div>
              ))}
            </div>
